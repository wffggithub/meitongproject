...
class ContactController extends ComController
{
    ...
    public function index($sid = 0, $p = 1)
    {
        ...
        if ($sid) {
            ...
            $where .= "and {$prefix}contact.sid in ($sids) ";
        }
        if ($keyword) {
            $where .= "and {$prefix}contact.title like '%{$keyword}%' ";
        }
        //默认按照时间降序
        $orderby = "t desc";
        if ($order == "asc") {
       ...
       /* $tree = new Tree($category);
        ...
        $count = $contact->where($where)->count();
        $list = $contact->field("{$prefix}contact.*,{$prefix}category.name")->where($where)->order($orderby)->join("{$prefix}category ON {$prefix}category.id = {$prefix}contact.sid")->limit($offset . ',' . $pagesize)->select();
    ...
    public function edit($id)
    {
        ...
        if ($contact) {
            ...
            /*$tree = new Tree($category);
            $str = "<option value=\$id \$selected>\$spacer\$name</option>"; //生成的形式
            $category = $tree->get_tree(0, $str, $contact['sid']);*/
            $this->assign('category', $category);//导航
    ...
    public function update($id = 0)
    {
        ...
        $data['company'] = isset($_POST['company']) ? $_POST['company'] : false;
        $data['addr'] = isset($_POST['addr']) ? $_POST['addr'] : false;
        $data['phone'] = isset($_POST['phone']) ? $_POST['phone'] : false;
        $data['content'] = isset($_POST['content']) ? htmlspecialchars($_POST['content']) : false;
